Public Class frmDangkiThongtinXuatkhoThanhpham
    Dim count1 As Integer = 0

    Dim T As Integer  'Page Count
    Dim k1 As Integer 'Get the Remainder  of the show ending the top of the list for the number of variables
    Dim g1 As Integer = 0 'By comparing the end of the event would show the number of clicks to obtain shares Wed
    Dim index As Integer = 0  'Show the number of list 

    Dim ExcelApp As Microsoft.Office.Interop.Excel.Application
    Dim ExcelBook As Microsoft.Office.Interop.Excel.Workbook
    Dim ExcelSheet As Microsoft.Office.Interop.Excel.Worksheet

    Dim count As Integer = 0
    Dim sql As String = "" ' Query
    Dim ds As Data.DataSet 'DataSet 
    Dim r As DataRow 'Column
    Dim nSeqNo As Integer

    Private Sub frmDangkiThongtinXuatkhoThanhpham_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        txtMaravaocong.Text = nprimUID.Trim       
        sql = "select * from TB_XUATKHO where nPrimUID = " & nprimUID
        ds = CMgrDataBase.GetInstance().ReturnDataSet(sql) 'DataSet    
        count = ds.Tables(0).Rows.Count

        If count > 0 Then
            r = ds.Tables(0).Rows(0)
            txtCoilID.Text = r("CoilID").ToString.Trim
            txtChungloai.Text = r("czChungloai").ToString.Trim
            txtBemat.Text = r("czBemat").ToString.Trim
            txtDoday.Text = r("czDoday").ToString.Trim
            txtDorong.Text = r("czRong").ToString.Trim
            txtChieudai.Text = r("czDai").ToString.Trim
            txtKhoiluongthuc.Text = r("cznetWeight").ToString.Trim
            txtKhoiluonggop.Text = r("czgrossweight").ToString.Trim
            txtGhichu.Text = r("czGhichu").ToString.Trim
            txtTenkhachhang.Text = r("czTenkhachhang").ToString.Trim
            txtManhanvien.Text = r("czManhanvien").ToString.Trim
        End If

    End Sub

    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Dim czChungloai As String = txtChungloai.Text
        Dim czBemat As String = txtBemat.Text
        Dim czDoday As String = txtDoday.Text
        Dim czRong As String = txtDorong.Text
        Dim czChieudai As String = txtChieudai.Text
        Dim czkhoiluongthuc As String = txtKhoiluongthuc.Text
        Dim czkhoiluonggop As String = txtKhoiluonggop.Text
        Dim czTenkhachhang As String = txtTenkhachhang.Text
        Dim czGhichu As String = txtGhichu.Text

        sql = "update TB_XUATKHO set czChungloai = '" + czChungloai + "', czBemat = '" + czBemat + "', czDoday = '" + czDoday + "', czRong = '" + czRong + "', czDai = '" + czChieudai + "', cznetWeight = " & czkhoiluongthuc & ", czgrossweight = " & czkhoiluonggop & ", czTenkhachhang = N'" + czTenkhachhang + "', czGhichu = N'" + czGhichu + "' where nPrimUID = " & nprimUID
        If MessageBox.Show("Bạn có muốn chỉnh sửa thông tin không?", "Xác nhận thông tin", MessageBoxButtons.OKCancel) = Windows.Forms.DialogResult.OK Then
            CMgrDataBase.GetInstance.ExecuteNonQuery(sql)
        End If
        Me.Close()
    End Sub

    Private Sub btnCancel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCancel.Click
        Me.Close()
    End Sub
End Class